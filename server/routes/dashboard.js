...
function get (req, res) {
    find(req.user.email, r => {
        ...
        else if(r.data.user.projects.length == 0 && r.data.user.invites.length == 0) res.render(`${__dirname}/../../views/dashboard`, { user: r.data.user, projects: [], invites: []});
        else {
            let projects = [];
            if(r.data.user.projects.length == 0) {
                let invites = [];
